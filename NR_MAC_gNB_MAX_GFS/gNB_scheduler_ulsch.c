                       ...
                       const uint16_t rssi)
{
  ...
  if (UE) {
    ...
    // Check if this HARQ process is used for Grant-Free
    if (gNB_mac->gf_manager.gf_enabled) {
      nr_gf_gnb_ue_config_t *gf_config = nr_gf_gnb_get_config_by_harq(gNB_mac, 
                                                                       UE->uid,  // or appropriate UE ID
                                                                       harq_pid);
      if (gf_config != NULL) {
        bool crc_valid = (sduP != NULL);
        int8_t sinr_db = (ul_cqi * 5 - 640) / 10;  
        
        LOG_I(NR_MAC, "[GF gNB] Received GF PUSCH from UE %04x, HARQ %d, CRC %s\n",
              rntiP, harq_pid, crc_valid ? "OK" : "FAIL");
        
        // Process Grant-Free reception
        nr_gf_gnb_process_rx(gNB_mac, UE->uid, harq_pid, crc_valid, 
                             sduP, sdu_lenP, sinr_db);
        
        // For GF, we handle HARQ differently, so return early
        return;
      }
    }
...
void post_process_ulsch(gNB_MAC_INST *nr_mac, post_process_pusch_t *pusch, NR_UE_info_t *UE, NR_sched_pusch_t *sched_pusch)
{
  ...
  // ============ ADD COMPREHENSIVE METRICS WITH FILE LOGGING ============
  /*
  // Calculate PRB utilization
  int total_bwp_prbs = current_BWP->BWPSize;
  float prb_utilization = (float)(sched_pusch->rbSize * 100.0) / total_bwp_prbs;
  */
  // Print to console
  /*
  printf("┌─ UL ALLOCATION [%4d.%2d] RNTI %04x ─────────────────────────────────────┐\n", frame, slot, UE->rnti);
  printf("│ FREQUENCY:  PRBs=%3d @ %3d | Utilization=%5.1f%% | BWP=%3d PRBs      │\n", 
         sched_pusch->rbSize, sched_pusch->rbStart, prb_utilization, total_bwp_prbs);
  printf("│ TIME:       Symbols=%2d @ %2d │\n",
         sched_pusch->tda_info.nrOfSymbols, sched_pusch->tda_info.startSymbolIndex);
  */
  // ============ SAVE TO FILE ============
  // Open file in append mode (creates file if it doesn't exist)
  /*
  FILE *log_file = fopen("/tmp/ul_allocation_log.csv", "a");
  if (log_file != NULL) {
    ...
    if (file_size == 0) {
      // Write CSV header
      fprintf(log_file, "PRBs,PRB_Start,PRB_Util_Percent,BWP_Size,");
      fprintf(log_file, "Symbols,Symbol_Start\n");
    }
    
    // Write data row
    fprintf(log_file, "%d,%d,%.2f,%d,",
            sched_pusch->rbSize, sched_pusch->rbStart, 
            prb_utilization, total_bwp_prbs);
    fprintf(log_file, "%d,%d\n",
            sched_pusch->tda_info.nrOfSymbols, sched_pusch->tda_info.startSymbolIndex);
    
    fclose(log_file);
  } else {
    LOG_E(NR_MAC, "Failed to open UL allocation log file\n");
  }*/
  // ============ END OF ADDITION ============

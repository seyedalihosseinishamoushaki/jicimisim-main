...
{
  uint8_t *pdu_ptr = pdu;
  uint8_t *pdu_end = pdu + pdu_size;
  LOG_I(NR_MAC, "[GF_DEBUG] Starting MAC PDU build, TBS=%d bytes\n", pdu_size);
  ...
  if (sched_info->BSR_reporting_active) {
    ...
    bool use_long_bsr = false;
    
    // Count LCGs with data
    int num_lcg_with_data = 0;
    for (int lcgid = 0; lcgid < NR_MAX_NUM_LCGID; lcgid++) {
      uint32_t lcg_bytes = 0;
      // Sum bytes for all LCs in this LCG
      for (int i = 0; i < mac->lc_ordered_list.count; i++) {
        nr_lcordered_info_t *lc_info = mac->lc_ordered_list.array[i];
        NR_LC_SCHEDULING_INFO *lc_sched_info = get_scheduling_info_from_lcid(mac, lc_info->lcid);
        if (lc_sched_info && lc_sched_info->LCGID == lcgid && lc_sched_info->LCID_buffer_remain > 0) {
          lcg_bytes += lc_sched_info->LCID_buffer_remain;
        }
      }
      if (lcg_bytes > 0) {
        num_lcg_with_data++;
        if (!use_long_bsr) {
          // Use first LCG for short BSR
          short_bsr.LcgID = lcgid;
          short_bsr.Buffer_size = nr_locate_BsrIndexByBufferSize(NR_SHORT_BSR_TABLE_SIZE, lcg_bytes);
        }
      }
    }
    
    // Decide BSR type based on available space and data
    int bsr_size_needed;
    if (num_lcg_with_data > 1) {
      // Would prefer Long BSR but check space
      bsr_size_needed = sizeof(NR_MAC_SUBHEADER_SHORT) + sizeof(NR_BSR_LONG) + num_lcg_with_data;
      use_long_bsr = (pdu_ptr + bsr_size_needed <= pdu_end);
    }
    
    if (!use_long_bsr) {
      // Use Short BSR
      bsr_size_needed = sizeof(NR_MAC_SUBHEADER_FIXED) + sizeof(NR_BSR_SHORT);
    }
    
    if (pdu_ptr + bsr_size_needed <= pdu_end) {
      if (use_long_bsr) {
        // Long BSR (for future use, currently using Short BSR for GF)
        LOG_W(NR_MAC, "[GF] Long BSR not implemented for GF, using Short BSR\n");
        use_long_bsr = false;
        bsr_size_needed = sizeof(NR_MAC_SUBHEADER_FIXED) + sizeof(NR_BSR_SHORT);
      }
      
      if (!use_long_bsr) {
        // Short BSR - use proper structure
        *(NR_MAC_SUBHEADER_FIXED *)pdu_ptr = (NR_MAC_SUBHEADER_FIXED){
          .R = 0, 
          .LCID = UL_SCH_LCID_S_BSR
        };
        pdu_ptr += sizeof(NR_MAC_SUBHEADER_FIXED);
        
        *(NR_BSR_SHORT *)pdu_ptr = short_bsr;
        pdu_ptr += sizeof(NR_BSR_SHORT);
  ...
  for (int idx = 0; idx < mac->lc_ordered_list.count && pdu_ptr < pdu_end; idx++) {
    nr_lcordered_info_t *lc_info = mac->lc_ordered_list.array[idx];
    ...
    int remaining_space = pdu_end - pdu_ptr;
    
    // Need at least space for subheader + 1 byte data
    if (remaining_space < sizeof(NR_MAC_SUBHEADER_SHORT) + 1) {
      break;  // Not enough space
    }
    
    // Determine subheader size and max SDU size
    int subheader_size;
    int max_sdu_size;
    bool use_short_header;
    
    // Reserve space for subheader
    if (remaining_space - sizeof(NR_MAC_SUBHEADER_LONG) > 0) {
      // We have space for either short or long header
      max_sdu_size = remaining_space - sizeof(NR_MAC_SUBHEADER_LONG);
      if (max_sdu_size <= 255) {
        use_short_header = true;
        subheader_size = sizeof(NR_MAC_SUBHEADER_SHORT);
        max_sdu_size = remaining_space - subheader_size;
      } else {
        use_short_header = false;
        subheader_size = sizeof(NR_MAC_SUBHEADER_LONG);
        max_sdu_size = remaining_space - subheader_size;
      }
    } else {
      // Only space for short header
      use_short_header = true;
      subheader_size = sizeof(NR_MAC_SUBHEADER_SHORT);
      max_sdu_size = remaining_space - subheader_size;
    }
    ...
    int sdu_request_size = (lc_sched_info->LCID_buffer_remain < max_sdu_size) ?
                          lc_sched_info->LCID_buffer_remain : max_sdu_size;
    
    if (sdu_request_size <= 0) {
      continue;
    }
    
    // Reserve space for subheader
    uint8_t *sdu_start = pdu_ptr + subheader_size;
    
    // Request SDU from RLC
    tbs_size_t bytes_read = nr_mac_rlc_data_req(mac->ue_id,
                                            mac->ue_id,
                                            false,             // gNB_index
                                            lcid,
                                            sdu_request_size,
    ...
    if (bytes_read > 0) {
      // Double-check if we need to switch header type based on actual size
      if (bytes_read > 255 && use_short_header) {
        // Need to switch to long header
        if (pdu_ptr + sizeof(NR_MAC_SUBHEADER_LONG) + bytes_read <= pdu_end) {
          // Move SDU data to accommodate longer header
          memmove(pdu_ptr + sizeof(NR_MAC_SUBHEADER_LONG), sdu_start, bytes_read);
          use_short_header = false;
          subheader_size = sizeof(NR_MAC_SUBHEADER_LONG);
        } else {
          // Not enough space for long header, truncate
          bytes_read = 255;
        }
      }
      
      // Write proper MAC subheader
      if (use_short_header) {
        *(NR_MAC_SUBHEADER_SHORT *)pdu_ptr = (NR_MAC_SUBHEADER_SHORT){
          .R = 0,
          .F = 0,
          .LCID = lcid,
          .L = (uint8_t)bytes_read
        };
      } else {
        *(NR_MAC_SUBHEADER_LONG *)pdu_ptr = (NR_MAC_SUBHEADER_LONG){
          .R = 0,
          .F = 1,
          .LCID = lcid,
          .L = htons((uint16_t)bytes_read)  // Network byte order!
        };
      }
      
      pdu_ptr += subheader_size + bytes_read;
      
      // Update buffer tracking
      lc_sched_info->LCID_buffer_remain -= bytes_read;
      
      LOG_I(NR_MAC, "[GF] Added SDU: LCID=%d, size=%d bytes, header_type=%s, remaining=%d\n",
            lcid, bytes_read, use_short_header ? "SHORT" : "LONG", 
            lc_sched_info->LCID_buffer_remain);
    }
  }
  
  // ========== Add padding if needed ==========
int remaining_bytes = pdu_end - pdu_ptr;
  if (remaining_bytes > 0) {
    // Write a SINGLE padding subheader
    *(uint8_t *)pdu_ptr = UL_SCH_LCID_PADDING;  // 0x3F
    pdu_ptr += 1;
    
    // The remaining bytes can be anything (gNB should ignore them after padding subheader)
    // Option A: Fill with zeros (most common)
    if (remaining_bytes > 1) {
      memset(pdu_ptr, 0x00, remaining_bytes - 1);
    }
    
    // Option B: Fill with more padding LCIDs (your current approach, but inefficient)
    // if (remaining_bytes > 1) {
    //   memset(pdu_ptr, UL_SCH_LCID_PADDING, remaining_bytes - 1);
    // }
    
    pdu_ptr = pdu_end;
    
    LOG_D(NR_MAC, "[GF] Added 1 padding subheader + %d padding bytes\n", remaining_bytes - 1);

  }  
  int total_bytes = pdu_ptr - pdu;
  
  LOG_I(NR_MAC, "[GF] MAC PDU built: %d bytes total\n", total_bytes);
  
#ifdef ENABLE_MAC_PAYLOAD_DEBUG
  LOG_I(NR_MAC, "[GF] MAC PDU dump:\n");
  log_dump(NR_MAC, pdu, total_bytes, LOG_DUMP_CHAR, "\n");
#endif
  
  // Verify the PDU we just built
  uint8_t *verify_ptr = pdu;
  int remaining = total_bytes;
  int sub_pdu_count = 0;
  bool valid = true;
  while (remaining > 0) {
    uint8_t lcid = verify_ptr[0] & 0x3F;
    uint16_t subhdr_len = 1;
    uint16_t payload_len = 0;
    
    LOG_I(NR_MAC, "[GF_DEBUG] SubPDU %d: LCID=0x%02x", sub_pdu_count, lcid);
    
    switch (lcid) {
      case UL_SCH_LCID_S_BSR:
        payload_len = 1;
        LOG_I(NR_MAC, " (Short BSR, payload=%d)", payload_len);
        break;
      case UL_SCH_LCID_PADDING:
        payload_len = 0;
        LOG_I(NR_MAC, " (Padding - rest of PDU is padding, stopping parse)\n");
        return valid;
      case 1 ... 32: // Data LCIDs
        if (verify_ptr[0] & 0x40) { // F=1 (long header)
          subhdr_len = 3;
          payload_len = (verify_ptr[1] << 8) + verify_ptr[2];
        } else { // F=0 (short header)
          subhdr_len = 2;  
          payload_len = verify_ptr[1];
        }
        LOG_I(NR_MAC, " (Data LCID, hdr=%d, payload=%d)", subhdr_len, payload_len);
        break;
      default:
        LOG_E(NR_MAC, "[GF_DEBUG] Unknown LCID 0x%02x!", lcid);
        break;
    }
    
    int total_subpdu_len = subhdr_len + payload_len;
    LOG_I(NR_MAC, ", total_len=%d, remaining=%d\n", total_subpdu_len, remaining);
    
    if (total_subpdu_len > remaining) {
      LOG_E(NR_MAC, "[GF_DEBUG] ERROR: SubPDU %d claims %d bytes but only %d remaining!\n",
            sub_pdu_count, total_subpdu_len, remaining);
      break;
    }
    
    verify_ptr += total_subpdu_len;
    remaining -= total_subpdu_len;
    sub_pdu_count++;
  }
...
/**
 * Check if current slot is a Grant-Free transmission occasion
 */
...
/**
 * Configure PUSCH PDU using predefined Grant-Free parameters
 * 
                                    ...
                                    int slot)
{
  ...
  pusch_pdu->nrOfLayers = 1;  // Single layer
  ...
  pusch_pdu->tbslbrm = 0;  // Disable TB size 
...
/**
 * Main Grant-Free scheduler function
 * 
 ...
 * 2. Optionally check if there's data to transmit
...
void nr_ue_schedule_grant_free(NR_UE_MAC_INST_t *mac, frame_t frame, int slot)
{
  ...
  // ========== Get ul_config and allocate PDU ==========
  ...
  // Note: Need to check which is the right mechanism to pass MAC PDU to PHY.
  ...
  // ========== Release ul_config and update stats ==========
...
/**
 * Handle HARQ feedback for Grant-Free transmissions
 * 
                            ...
                            int harq_feedback)
{
  ...
  if (harq_feedback == 1) {
    ...
    // Update statistics
    // gf->harq_ack_count++;  // To add later
...
/**
 *      to fetch data to be transmitted from RLC, place it in the ULSCH PDU buffer
